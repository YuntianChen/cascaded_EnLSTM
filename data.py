...
class WelllogDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, input_dim, output_dim, train_len):  # 默认excel中前5口井训练，第6口检测
        ...
        for i in range(WELL):
            ...
            filename = 'data/vertical_all_A{}.csv'.format(i+1)
        ...
        for feature in COLUMNS:
            self.scaler[feature] = preprocessing.StandardScaler().fit(self.dataset[feature])
        # self.target_scaler = preprocessing.StandardScaler().fit(self.dataset[config.columns_target].values)
    ...
    #        self.data_all[items-1][config.columns_target] = self.test_data[items][config.columns_target].values

    def train_dataset(self):
        input_data = []
        target_data = []
        # selected = COLUMNS[:config.input_dim+config.output_dim]
        for items in TRAIN_ID:
            ...
            for feature in COLUMNS[self.input_dim:self.input_dim+self.output_dim]:
                target_.append(self.scaler[feature].transform(data[feature]).values)
            target_ = np.array(list(make_dataset(
                self.target_scaler.transform(data[COLUMNS[self.input_dim:self.input_dim + config.output_dim]].values), self.train_len)))
    ...
    def test_dataset(self, index):
        ...
        data = copy.copy(self.data_all[index-1])
        ...
        target_ = self.target_scaler.transform(data[config.columns_target].values)
